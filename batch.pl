...
$simulation{"LoRaWAN"}{"cmdline"} = "./LoRaWAN.pl 12 10000 2 %f 1";
...
$simulation{"5RX2-SF"}{"cmdline"} = "./LoRaWAN-5RX2-SF.pl 12 10000 2 %f 1";
...
$simulation{"5RX2-eqNodesperBnd"}{"cmdline"} = "./LoRaWAN-5RX2-eqNodesperBnd.pl 12 10000 2 %f 1";
...
$simulation{"5RX2-eqNodesperCh"}{"cmdline"} = "./LoRaWAN-5RX2-eqNodesperCh.pl 12 10000 2 %f 1";
...
$simulation{"5RX2-eqToAperBnd"}{"cmdline"} = "./LoRaWAN-5RX2-eqToAperBnd.pl 12 10000 2 %f 1";
...
$simulation{"10pRX1"}{"cmdline"} = "./LoRaWAN-10pRX1.pl 12 10000 2 %f 1";
...
for (my $nodes=$start; $nodes<=$end; $nodes+=100){
	print STDERR "------------------------------------------\n";
	my %sf = ();
	my %data_sf = ();
	my %energy = ();
	...
	for (my $i=1; $i<=10; $i++){
		...
		if ($oops == 0){
			foreach my $alg (sort keys %simulation){
				...
				printf STDERR "# %s %d %d %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n", $alg, $nodes, $i, $sf{$alg}/$repeats, $energy{$alg}/$repeats, $pdr{$alg}/$repeats, $prr{$alg}/$repeats, $dropped{$alg}/$repeats, $rx1{$alg}/$repeats, $rx2{$alg}/$repeats, $fair{$alg}/$repeats, $stdv{$alg}/$repeats, $trans{$alg}/$repeats;
			}
		}else{
			$i -= 1;
		}
