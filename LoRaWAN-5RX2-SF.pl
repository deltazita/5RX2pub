	...
	if ($sel =~ /^[0-9]/){ # if the packet is an uplink transmission
		...
		if ((scalar @$gw_rc > 0) && ($nconfirmed{$sel} == 1)){ # if at least one gateway received the pkt -> successful transmission
			...
			if (defined $sel_gw){
				...
				if (defined $sel_gw){
					# SF7, 10, and 11 use the old RX2 channel ($rx2ch[4]), the rest use the 3 first new RX2 channels ($rx2ch[0-2])
					my ($sf, $ch, $bnd) = ($rx2sf, 0, "47b"); # band: 54 for 869525000, 47b for the other 4 channels
					if (($sel_sf == 7) || ($sel_sf == 10) || ($sel_sf == 11)){
						$ch = $rx2ch[-1];
						$bnd = "54";
					}elsif ($sel_sf == 8){
						$ch = $rx2ch[0];
