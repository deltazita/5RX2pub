...
my %band = (868000000=>"48", 868200000=>"48", 868400000=>"48", 867000000=>"47", 867200000=>"47", 867400000=>"47", 867600000=>"47", 867800000=>"47", 865600000=>"47b", 866200000=>"47b", 866800000=>"47b", 867400000=>"47b", 869525000=>"54"); # band name per channel
...
while (1){
	...
	if ($sel =~ /^[0-9]/){ # if the packet is an uplink transmission
		...
		if ((scalar @$gw_rc > 0) && ($nconfirmed{$sel} == 1)){ # if at least one gateway received the pkt -> successful transmission
			...
			# check RX1
			my ($sel_gw, $sel_p, $dn_ch) = gs_policy($sel, $sel_sta, $sel_end, $sel_ch, $sel_sf, $gw_rc, 1);
			if (defined $sel_gw){
				my ($ack_sta, $ack_end) = ($sel_end+1, $sel_end+1+airtime($sel_sf, $overhead_d));
				$total_down_time += airtime($sel_sf, $overhead_d);
				$rwindow = 1;
				print "# gw $sel_gw will transmit an ack to $sel (RX$rwindow) (channel $dn_ch)\n" if ($debug == 1);
				$gresponses{$sel_gw} += 1;
				push (@{$gunavailability{$sel_gw}}, [$ack_sta, $ack_end, $dn_ch, $sel_sf, "d"]);
				$gdc{$sel_gw}{$band{$dn_ch}} = $ack_end+airtime($sel_sf, $overhead_d)*9;
				...
				splice(@sorted_t, $i, 0, [$new_name, $ack_sta, $ack_end, $dn_ch, $sel_sf, $appacked{$sel}]);
				push (@{$gdest{$sel_gw}}, [$sel, $sel_end+$rwindow, $sel_sf, $rwindow, $dn_ch, -1]);
			}else{
				# check RX2
				$no_rx1 += 1;
				($sel_gw, $sel_p, $dn_ch) = gs_policy($sel, $sel_sta, $sel_end, $sel_ch, $sel_sf, $gw_rc, 2);
				if (defined $sel_gw){
...
# 			$nperiod{$sel} = random_poisson(1, $period) if ($fixed_packet_rate == 0);
			...
			$nconsumption{$sel} += $at * $Ptx_w[$nptx{$sel}] + (airtime($sel_sf, $npkt{$sel})+1) * $Pidle_w if ($sel_sta < $sim_time);
...
# 				$nperiod{$sel} = random_poisson(1, $period) if ($fixed_packet_rate == 0);
		...
		# remove the examined tuple from gw unavailability
		my @indices = ();
		my $index = 0;
...
sub gs_policy{ # gateway selection policy
	...
	my $bnd = undef;
	if ($win == 1){
		# the first 4 uplink channels are mapped to the last downlink channel
		# the last 4 uplink channels are mapped to the first 4 downlink channels
		if (($band{$sel_ch} eq "48") || ($sel_ch eq "867000000")){
			$sel_ch = $rx2ch[-1];
			$bnd = "54";
		}else{
			my $idx = first { $channels[$_] eq $sel_ch } 0..$#channels; # get the index of $sel_ch in @channels
			$sel_ch = $rx2ch[$idx-4];
			$bnd = "47b";
		}
	}else{
...
sub read_data{
	...
	foreach my $node (@nodes){
		my ($n, $x, $y) = @$node;
		$ncoords{$n} = [$x, $y];
		$nrx2ch{$n} = int(rand(5)); # equally distribute nodes to 5 RX2 channels
