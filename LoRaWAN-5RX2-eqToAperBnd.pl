...
my $fixed_packet_rate = 0; # send packets periodically with a fixed rate per node (=1) or at random per node (=0)
...
sub read_data{
	...
	my %band_occupancy = (0 => 0, 1 => 0);
	my %chan_occupancy = (0 => 0, 1 => 0, 2 => 0, 3 => 0);
	foreach my $node (@nodes){
		...
		if ($conf_num > 0){
			...
			my $sel_band = undef;
			my $sel_chan = undef;
			my $min_band_occupancy = 9999999999999;
			my $min_chan_occupancy = 9999999999999;
			foreach my $bnd (keys %band_occupancy){ # select the band with the least occupancy 
				if ($band_occupancy{$bnd} < $min_band_occupancy){
					$sel_band = $bnd;
					$min_band_occupancy = $band_occupancy{$bnd};
				}
			}
			if ($sel_band == 1){
				$nrx2ch{$n} = 4;
			}else{
				foreach my $ch (keys %chan_occupancy){
					if ($chan_occupancy{$ch} < $min_chan_occupancy){
						$sel_chan = $ch;
						$min_chan_occupancy = $chan_occupancy{$ch};
					}
				}
				$nrx2ch{$n} = $sel_chan;
				$chan_occupancy{$sel_chan} += 3600/$nperiod{$n};
			}
			$band_occupancy{$sel_band} += 3600/$nperiod{$n};
		}else{
			$nconfirmed{$n} = 0;
		}
