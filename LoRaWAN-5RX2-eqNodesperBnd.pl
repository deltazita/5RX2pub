	...
	if ($sel =~ /^[0-9]/){ # if the packet is an uplink transmission
		...
		if ((scalar @$gw_rc > 0) && ($nconfirmed{$sel} == 1)){ # if at least one gateway received the pkt -> successful transmission
			...
			if (defined $sel_gw){
				...
				if (defined $sel_gw){
					# SF7 and 8 use the old RX2 channel ($rx2ch[4]), the rest use the 4 new RX2 channels ($rx2ch[0-3])
...
sub read_data{
	...
	my $all_nodes = (scalar @nodes); # we assume that we don't know the number of nodes with confirmed transmissions
	my $band_popul_limit = ceil($all_nodes/2);
	my %band_popul = (0 => 0, 1 => 0); # devide the nodes in 2 parts
	my $b54_popul_limit = ceil($all_nodes/8);
	my %b54_popul = (0 => 0, 1 => 0, 2 => 0, 3 => 0);
	foreach my $node (@nodes){
		my ($n, $x, $y) = @$node;
		$ncoords{$n} = [$x, $y];
	}
	foreach my $n (sort {$nperiod{$a} <=> $nperiod{$b}} keys %nperiod){
		my $band_selection = 0; #int(rand(2));
		if ($band_popul{$band_selection} >= $band_popul_limit){
			$band_selection = 1; #int(rand(2));
		}
		$band_popul{$band_selection} += 1;
		if ($band_selection == 1){
			$nrx2ch{$n} = int(rand(4)); # equally distribute nodes to 4 RX2 channels of band 54
			while ($b54_popul{$nrx2ch{$n}} >= $b54_popul_limit){
				$nrx2ch{$n} = int(rand(4));
			}
			$b54_popul{$nrx2ch{$n}} += 1;
		}else{
			$nrx2ch{$n} = 4; # give 869.525
		}
		@{$overlaps{$n}} = ();
